...
public class RoomGenerator6 : MonoBehaviour
{
    ...
    private void GenerateWallWithDoor(GameObject parent, Room currentRoom, Vector3 wallCenter, Vector3 wallScale, int direction, float wallFullHeight)
    {
        ...
        // Assign materials if needed, example:
         wallSegment1.GetComponent<Renderer>().material.color = Color.grey;
    ...
    private void ConnectRoomsWithCorridors()
    {
        ...
        while (unconnected.Count > 0)
        {
            ...
            // This loop should call your NEW pathfinder (e.g., A*)
            // which returns List<Vector3> pathPoints.
            // The logic needs adapting to find the best path based on the new pathfinder's output.
            // For now, it still uses CalculateLPath.
                ...
                foreach (Room targetRoom in unconnected)
                {
                    foreach (Door sourceDoor in sourceRoom.doors)
                    {
                        ...
                        foreach (Door targetDoor in targetRoom.doors)
                        {
                            ...
                            // Basic facing check (optional but can help)
                            // Vector3 roomToRoomDir = (targetRoom.position - sourceRoom.position).normalized;
                            // if (Vector3.Dot(GetDirectionVector(sourceDoor.direction), roomToRoomDir) < 0f) continue;
                            // if (Vector3.Dot(GetDirectionVector(targetDoor.direction), -roomToRoomDir) < 0f) continue;

                            // *** CALL OLD L-PATH CALCULATOR ***
                            ...
                            if (currentPathResult != null && currentPathResult.Points.Count >= 2)
                            {
                                ...
                                {
                                    // *** Overlap check needs adapting if path structure changes drastically ***
                                    // For now, it checks the L-Path result
            ...
            {
                // *** CALL NEW BuildCorridorFromPath ***
                // Pass the points list from the (currently L-Path) result
    ...
    // --- Attempt to connect the last room back to the first room --- (Keep this part from before)
    Debug.Log("Attempting final loop connection: Last room to First room...");
    // ... (rest of your last-to-first connection code) ...
    ...
    // NOTE: Replace this with a proper pathfinder (e.g., A*) for multi-corner paths.
    private PathCalculationResult CalculateLPath(Door startDoor, Door endDoor)
    {
     ...
     // NOTE: Needs careful review/update if path generation changes significantly.
     // Currently checks segments and corners implied by the point list.
    ...
    // ------------------------------------------------------------------
    // VVVVVV  NEW BuildCorridorFromPath and its Helpers VVVVVV
    // ------------------------------------------------------------------

    /// <summary>
    /// Builds the corridor geometry from a list of path points, handling multiple corners.
    /// Assumes pathPoints are orthogonal and represent the corridor centerline.
    /// </summary>
    /// <param name="pathPoints">List of Vector3 points defining the path centerline.</param>
    /// <param name="width">Width of the corridor.</param>
    /// <param name="height">Height of the corridor.</param>
    /// <param name="thickness">Thickness of walls, floor, ceiling.</param>
    ...
    private Bounds CreateCorridorSegment(Transform parent, Vector3 effectiveStart, Vector3 effectiveEnd, float width, float height, float thickness)
    {
        ...
        ceiling.GetComponent<Renderer>().material.color = Color.Lerp(Color.grey, Color.black, 0.5f);

        // Side Walls
        Vector3 wallCenterY = new Vector3(segmentCenter.x, height / 2, segmentCenter.z);
        GameObject wall1 = GameObject.CreatePrimitive(PrimitiveType.Cube); wall1.name = "WallSegment1"; wall1.layer = LayerMask.NameToLayer("NotWalkable"); wall1.transform.parent = parent; wall1.GetComponent<Renderer>().material.color = Color.grey;
        GameObject wall2 = GameObject.CreatePrimitive(PrimitiveType.Cube); wall2.name = "WallSegment2"; wall2.layer = LayerMask.NameToLayer("NotWalkable"); wall2.transform.parent = parent; wall2.GetComponent<Renderer>().material.color = Color.grey;
        if (isHorizontal) {
         ...
         // Return approximate bounds - update if more accuracy needed
    ...
    private void CreateOuterCornerWalls(Transform parent, Vector3 p_before_corner, Vector3 cornerPoint, Vector3 p_after_corner, float width, float height, float thickness)
    {
        ...
        // --- Wall 1 (Parallel to Outgoing) ---
        GameObject outerWall1 = GameObject.CreatePrimitive(PrimitiveType.Cube);
        outerWall1.name = "OuterCornerWall_1"; outerWall1.transform.parent = parent; outerWall1.layer = LayerMask.NameToLayer("NotWalkable"); outerWall1.GetComponent<Renderer>().material.color = Color.grey;
        ...
        outerWall1.transform.Rotate(0f, 90f, 0f, Space.Self); // <--- ADD THIS ROTATION BACK

        // --- Wall 2 (Parallel to Incoming) ---
        GameObject outerWall2 = GameObject.CreatePrimitive(PrimitiveType.Cube);
        outerWall2.name = "OuterCornerWall_2"; outerWall2.transform.parent = parent; outerWall2.layer = LayerMask.NameToLayer("NotWalkable"); outerWall2.GetComponent<Renderer>().material.color = Color.grey;
        // Use the adjusted position
        ...
        outerWall2.transform.Rotate(0f, 90f, 0f, Space.Self); // <--- ADD THIS ROTATION BACK

        // Just a little reminder: Rotating walls that have different width/thickness scales like this
        // can sometimes make textures look stretched or cause slight visual gaps/overlaps
        // if the scaling isn't perfectly matched to the post-rotation orientation. But let's see how this looks!
    }
